...
- SMSBower 轮询或手动完成遇到 `NO_ACTIVATION` 时，会通过标准 `getStatus` 再确认。若上游确认订单不存在且本地当前激活已收到短信，则将本地订单标记为已完成，保留金额和短信，并记录 `upstream_missing` 原因；这不代表新增扣费、退款或再次成功操作了上游。没有收码证据或确认失败时，不会仅凭该错误完成订单。
